...
import { Plus, X, Key, Send, Terminal } from "lucide-react";
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="rounded-lg border border-primary/20 bg-primary/5 p-4 space-y-2">
        <p className="text-sm font-semibold text-foreground flex items-center gap-2"><Send className="h-4 w-4 text-primary" /> {t("wizard.get_token")}</p>
        <ol className="text-sm text-muted-foreground space-y-1 list-decimal list-inside">
          <li>{t("wizard.tg_step1")} <a href="https://t.me/BotFather" target="_blank" rel="noopener" className="text-primary underline">@BotFather</a></li>
          <li>{t("wizard.tg_step2_send")} <code className="text-xs bg-muted px-1 py-0.5 rounded">/newbot</code> {t("wizard.tg_step2_follow")}</li>
          <li>{t("wizard.tg_step3")}</li>
        </ol>
      </div>

      <div className="space-y-2">
        <Label className="flex items-center gap-1.5 text-sm"><Key className="h-3.5 w-3.5" /> {t("wizard.bot_token")} *</Label>
        <Input
          type="password"
      ...
      <div className="space-y-3">
        <Label className="flex items-center gap-1.5 text-sm"><Terminal className="h-3.5 w-3.5" /> {t("wizard.bot_commands")}</Label>
        <div className="space-y-2">
